...
# UPDATE TIMESTEPS 
currentNumInf<-length(current_genotypes[,1])
meanNumNewInfPerTimestep<-0.45
probClearancePerTimestep<-0.025
pt<-1

#eir_data$time<-eir_data$time+100
...
for (t in 101:135) {  
  ...
  if (t==127) {
    # write to text file for investigations
    infs001<-current_genotypes
  }
}

meanNumNewInfPerTimestep<-0.0001 #2
...
for (t in 136:155) {  
  ...
  if (t==154) {
    # write to text file for investigations
    infs10<-current_genotypes
  }
  
}
#############################
#output files
write.table(infs0,"crossT0_seas.txt", row.names = F)
write.table(infs10,"dry_seas.txt", row.names = F)
write.table(infs01,"crossT1_seas.txt", row.names = F) #dry
write.table(infs001,"crossT2_seas.txt", row.names = F) #wet
...
cases20<-read.table("dry_seas.txt", header=T, row.names = NULL)

cases2<-infs001 #wet season
cases20<-infs01 #dry season

# create case files from both the crossectional surveys
# remove duplicates
cases3<-cases2[!duplicated(cases2[,c("location.id", "pID","locx","locy")]),]
cases30<-cases20[!duplicated(cases20[,c("location.id", "pID","locx","locy")]),]
...
#save case file
cases30$status<-1
cases40<-subset(cases30, select = c(location.id, pID, locx, locy, status))
write.table(cases40,"casesT_dryseas.txt", row.names = F)
...
cases60<-anti_join(homesteads, cases40, by = c("location.id"))
...
cases50<-merge(cases40, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases50$status[is.na(cases50$status)]<-0
write.table(cases50,"case_ctrlT_dryseas.txt", row.names = F, col.names = F)
...
#controls
controls20<-anti_join(populate, cases40, by = c("location.id", "pID"))
controls20$status<-1
write.table(controls20,"ctrlsT_dryseas.txt", row.names = F)
...
write.table(pop_file,"pop_file_seas.txt", row.names = F)
...
# case file for flexscan
cases_total20<-ddply(cases40, .(location.id), summarize,  case=sum(status))
cases_total20<-merge(cases_total20, pop_file, by="location.id", all=T)
cases_total20$case[is.na(cases_total20$case)]<-0

write.table(cases_total20,"cases_popT_dryseas.txt", row.names = F, col.names = F)
