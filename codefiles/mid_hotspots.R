...
# UPDATE TIMESTEPS 
currentNumInf<-length(current_genotypes[,1])
meanNumNewInfPerTimestep<-0.0375
...
meanNumNewInfPerTimestep<-0.0365
probClearancePerTimestep<-0.025
...
#############################
#output files
write.table(infs0,"crossT0_mid.txt", row.names = F)
write.table(infs1,"crossT1_mid.txt", row.names = F)

meanNumNewInfPerTimestep<-0.0363
probClearancePerTimestep<-0.025
...
write.table(infs2,"crossT2_mid.txt", row.names = F)
write.table(infs3,"crossT3_mid.txt", row.names = F)
...
write.table(cases4,"casesT03_mid.txt", row.names = F)
...
write.table(cases5,"case_ctrlT03_mid.txt", row.names = F, col.names = F)

#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT03_mid.txt", row.names = F)

#population in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_mid.txt", row.names = F)
...
write.table(cases_total2,"cases_popT03_mid.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT_mid.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT_mid.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT_mid.txt", row.names = F)
...
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT_mid.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT3y_mid.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT3y_mid.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT3y_mid.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_mid.txt", row.names = F)
...
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT3y_mid.txt", row.names = F, col.names = F)
