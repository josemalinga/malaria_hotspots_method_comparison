  ...
  for (i in 1:currentNumInf){
    ...
    for (q in 1:nrow(infHousesHot)){    
      if (currenthome == infHousesHot[q,"location.id"]) {
        meanNew<-meanNew * hotspotFactor * infHousesHot[q,"hotFactor"]
      }
    }
...
updateCurrentGenotypes<-function(t){
  ...
  if (length(current_genotypes[,1])>5000){
   nt<-seq(1:length(current_genotypes[,1]))
    nt <- sample(nt, 5000,replace=FALSE)
...
#probRecombination<-para_grid[taskid,2]
#meanNumNewInfPerTimestepI<-para_grid[taskid,2]
#meanNumNewInfPerTimestepR<-para_grid[taskid,3]
...
infHousesHot<-read.table("hotHomes_D.txt",header=TRUE)
colnames(infHousesHot)[1:5]<-c("locx","locy","location.id", "hotFactor","hotspot")
...
# UPDATE TIMESTEPS 
currentNumInf<-length(current_genotypes[,1])
meanNumNewInfPerTimestep<-0.0378
...
#############################
#output files
write.table(infs0,"crossT0_D.txt", row.names = F)
write.table(infs1,"crossT1_D.txt", row.names = F)

meanNumNewInfPerTimestep<-0.0364
probClearancePerTimestep<-0.025
...
write.table(infs2,"crossT2_D.txt", row.names = F)
write.table(infs3,"crossT3_D.txt", row.names = F)
...
write.table(cases4,"casesT03_D.txt", row.names = F)
...
write.table(cases5,"case_ctrlT03_D.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT03_D.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_D.txt", row.names = F)
...
write.table(cases_total2,"cases_popT03_D.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT_D.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT_D.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT_D.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_D.txt", row.names = F)
...
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT_D.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT3y_D.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT3y_D.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT3y_D.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_D.txt", row.names = F)
...
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT3y_D.txt", row.names = F, col.names = F)
