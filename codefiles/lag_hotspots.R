...
# UPDATE TIMESTEPS 
currentNumInf<-length(current_genotypes[,1])
meanNumNewInfPerTimestep<-0.042
...
# running the model
meanNumNewInfPerTimestep<-0.040
probClearancePerTimestep<-0.025
...
#############################
#output files
write.table(infs0,"crossT0_lag.txt", row.names = F)
write.table(infs1,"crossT1_lag.txt", row.names = F)
...
write.table(infs2,"crossT2_lag.txt", row.names = F)
write.table(infs3,"crossT3_lag.txt", row.names = F)
...
write.table(cases4,"casesT03_lag.txt", row.names = F)
...
write.table(cases5,"case_ctrlT03_lag.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT03_lag.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_lag.txt", row.names = F)
...
write.table(cases_total2,"cases_popT03_lag.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT_lag.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT_lag.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT_lag.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_lag.txt", row.names = F)
...
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT_lag.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT3y_lag.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT3y_lag.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT3y_lag.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_lag.txt", row.names = F)
...
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT3y_lag.txt", row.names = F, col.names = F)
