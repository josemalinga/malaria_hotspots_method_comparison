...
#data preparation
#set seed - for different replicates
...
#seonality/EIR data
eir_data<-read.table("codefiles/eir_data_seas.txt", header=T, row.names = NULL)
#eir_data<-read.table("/Users/dukeghi/Documents/SCICORE Files/Malaria Hotspots/simulations_sep/eir_data.txt", header=T, row.names = NULL)
...
# UPDATE TIMESTEPS 
currentNumInf<-length(current_genotypes[,1])
meanNumNewInfPerTimestep<-0.005
...
for (t in 147:219) {  
  meanNumNewInfPerTimestep<-0.008
  probClearancePerTimestep<-0.025
  meanNumNewInfPerTimestep<-meanNumNewInfPerTimestep*eir_data$EIRscaled[eir_data$time==t]
  newInf<-getNewInfections(t) 
  #newInf<-checkNewInfections(newInf)
  ...
  if (t==165) {
    ...
    infs0w<-current_genotypes
  }
  
  if (t==185) {
    # write to text file for investigations
    infs0d<-current_genotypes
  }
...
#############################
#output files
write.table(infs0w,"crossT0_seas.txt", row.names = F)
write.table(infs0d,"crossT0_dry.txt", row.names = F)

meanNumNewInfPerTimestep<-0.010
probClearancePerTimestep<-0.025

for (t in 220:287) {  
  meanNumNewInfPerTimestep<-0.060
  probClearancePerTimestep<-0.025
  meanNumNewInfPerTimestep<-meanNumNewInfPerTimestep*eir_data$EIRscaled[eir_data$time==t]
  newInf<-getNewInfections(t) 
  #newInf<-checkNewInfections(newInf)
  ...
  if (t==238) {
    # write to text file for investigations
    infs1w<-current_genotypes
  }
  
  if (t==264) {
    # write to text file for investigations
    infs1d<-current_genotypes
  }
...
#############################
#output files
write.table(infs1w,"crossT1_seas.txt", row.names = F)
write.table(infs1d,"crossT1_dry.txt", row.names = F)

meanNumNewInfPerTimestep<-0.025
probClearancePerTimestep<-0.025

for (t in 288:365) {  
  meanNumNewInfPerTimestep<-0.048
  probClearancePerTimestep<-0.025
  meanNumNewInfPerTimestep<-meanNumNewInfPerTimestep*eir_data$EIRscaled[eir_data$time==t]
  newInf<-getNewInfections(t) 
  #newInf<-checkNewInfections(newInf)
  ...
  if (t==311) {
    # write to text file for investigations
    infs2w<-current_genotypes
  }
  
  if (t==332) {
    # write to text file for investigations
    infs2d<-current_genotypes
  }
...
#############################
#output files
write.table(infs2w,"crossT3_seas.txt", row.names = F)
write.table(infs2d,"crossT3_dry.txt", row.names = F)
...
write.table(cases4,"casesT03_seas.txt", row.names = F)
...
write.table(cases5,"case_ctrlT03_seas.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT03_seas.txt", row.names = F)
#population in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_file_T03_seas.txt", row.names = F)
...
write.table(cases_total2,"cases_popT03_seas.txt", row.names = F, col.names = F)

#3 months dry - that is 3 months after the wet season
cases2<-infs0d
# remove duplicates
cases3<-cases2[!duplicated(cases2[,c("location.id", "pID","locx","locy")]),]
#plot(cases3$locx, cases3$locy, col="darkblue", pch=20, cex=2, main="cases")
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT03_dry.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT03_dry.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT03_dry.txt", row.names = F)
#population in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_fileT03_dry.txt", row.names = F)
# case file for flexscan
cases_total2<-ddply(cases4, .(location.id), summarize,  case=sum(status))
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT03_dry.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT_seas.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT_seas.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT_seas.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_fileT_seas.txt", row.names = F)
# case file for flexscan
cases_total2<-ddply(cases4, .(location.id), summarize,  case=sum(status))
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT_seas.txt", row.names = F, col.names = F)


#1 year for dry season
cases2<-infs1d
# remove duplicates
cases3<-cases2[!duplicated(cases2[,c("location.id", "pID","locx","locy")]),]
#plot(cases3$locx, cases3$locy, col="darkblue", pch=20, cex=2, main="cases")
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT_dry.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT_dry.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT_dry.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_fileT_dry.txt", row.names = F)
# case file for flexscan
cases_total2<-ddply(cases4, .(location.id), summarize,  case=sum(status))
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT_dry.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT3y_seas.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT3y_seas.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT3y_seas.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_fileT3y_seas.txt", row.names = F)
# case file for flexscan
cases_total2<-ddply(cases4, .(location.id), summarize,  case=sum(status))
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT3y_seas.txt", row.names = F, col.names = F)
...
cases3$status<-1
cases4<-subset(cases3, select = c(location.id, pID, locx, locy, status))
write.table(cases4,"casesT3y_dry.txt", row.names = F)
# get population estimates and create control files
populate <- homesteads %>% slice(rep(1:n(), each = numPple))
names(populate)[1]<-"location.id"
setDT(populate)[, pID := seq_len(.N), by = location.id]
...
cases5<-merge(cases4, populate, by=c("location.id","locx","locy", "pID"), all=T)
cases5$status[is.na(cases5$status)]<-0
write.table(cases5,"case_ctrlT3y_dry.txt", row.names = F, col.names = F)
#controls
controls2<-anti_join(populate, cases4, by = c("location.id", "pID"))
controls2$status<-1
write.table(controls2,"ctrlsT3y_dry.txt", row.names = F)
#populcation in each household
populate$num<-1
pop_file<-ddply(populate, .(location.id), summarize,  pop=sum(num))
write.table(pop_file,"pop_fileT3y_dry.txt", row.names = F)
# case file for flexscan
cases_total2<-ddply(cases4, .(location.id), summarize,  case=sum(status))
cases_total2<-merge(cases_total2, pop_file, by="location.id", all=T)
cases_total2$case[is.na(cases_total2$case)]<-0
write.table(cases_total2,"cases_popT3y_dry.txt", row.names = F, col.names = F)
